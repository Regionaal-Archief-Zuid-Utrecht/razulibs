...
class Identifiers:
    ...
    def make_s3_path_from_id(self, id: str) -> str:
        """Generate an S3 key from an object ID, like 'nl-wbdrazu/g0321/661/000/000/nl-wbdrazu-g0321-661-4.meta.json'."""

        millions = int(id) // 1000000
        thousands = int(id) % 1000000 // 1000
        numbered_path = f"{str(millions).zfill(3)}/{str(thousands).zfill(3)}/"
        return f"{self.config.razu_file_id}/{self.config.archive_creator_id}/{self.config.archive_id}/{numbered_path}"
